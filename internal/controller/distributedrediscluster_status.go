...
func buildClusterStatus(clusterInfos *redisutil.ClusterInfos, pods *corev1.PodList,
...
func compareStatus(old, new *redisv1alpha1.DistributedRedisClusterStatus, reqLogger logr.Logger) bool {
	if helper.CompareStringValue("ClusterStatus", string(old.Status), string(new.Status), reqLogger) {
		return true
	}

	if helper.CompareStringValue("ClusterStatusReason", old.Reason, new.Reason, reqLogger) {
		return true
	}

	if helper.CompareInt32("NumberOfMaster", old.NumberOfMaster, new.NumberOfMaster, reqLogger) {
		return true
	}

	if helper.CompareInt32("len(Nodes)", int32(len(old.Nodes)), int32(len(new.Nodes)), reqLogger) {
		return true
	}

	if helper.CompareStringValue("restoreSucceeded", string(old.Restore.Phase), string(new.Restore.Phase), reqLogger) {
		return true
	}
...
func compareNodes(nodeA, nodeB *redisv1alpha1.RedisClusterNode, reqLogger logr.Logger) bool {
	if helper.CompareStringValue("Node.IP", nodeA.IP, nodeB.IP, reqLogger) {
		return true
	}
	if helper.CompareStringValue("Node.MasterRef", nodeA.MasterRef, nodeB.MasterRef, reqLogger) {
		return true
	}
	if helper.CompareStringValue("Node.PodName", nodeA.PodName, nodeB.PodName, reqLogger) {
		return true
	}
	if helper.CompareStringValue("Node.Port", nodeA.Port, nodeB.Port, reqLogger) {
		return true
	}
	if helper.CompareStringValue("Node.Role", string(nodeA.Role), string(nodeB.Role), reqLogger) {
		return true
	}
