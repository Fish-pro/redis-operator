...
func (r *Redis) AddFlags(fs *flag.FlagSet) {
	...
	fs.UintVar(&r.MaxMemory, "max-memory", RedisMaxMemoryDefault, "redis max memory")
	...
	fs.StringVar(&r.ConfigFiles, "config-file", "", "Location of redis configuration file that will be include in the ")
}
